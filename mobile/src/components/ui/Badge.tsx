...
export function Badge({ label, variant = 'gray', classification, small = false, animated: withBounce = false }: BadgeProps) {
  ...
  // Bounce entrance con RN Animated (no requiere worklets)
  const scale = useRef(new Animated.Value(withBounce ? 0 : 1)).current;
  ...
  return (
    <Animated.View style={[styles.base, { backgroundColor: bg }, small && styles.small, { transform: [{ scale }] }]}>
...
const styles = StyleSheet.create({
  ...
  small:     { paddingHorizontal: spacing[1], paddingVertical: 1 },
